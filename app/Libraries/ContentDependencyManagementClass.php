...
class ContentDependencyManagementClass 
{
	...
	public function setContentWithTitleChanged($id, $content, $contentTitle, $sharedByName, $sharedByEmail, $contentTs){
	    //PANKAJ--Title Updated By
	    ...
	    if(isset($userContent)){
            if(!empty($userContent->content_title) && $userContent->content_title != $contentTitle){
                $appuserData = Appuser::byId($userContent->appuser_id)->first();
        		...
        		$appendedStr .= 'Title changed from '.$userContent->content_title;
    ...
    public function addEditContent($id, $content,$contentTitle='No Title', $contentTypeId, $folderId, $sourceId, $tagsArr, $isMarked, $createTs, $updateTs, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmail = "",  $syncWithCloudCalendarGoogle = 0, $syncWithCloudCalendarOnedrive = 0, $isRemoved = 0, $removedAt = NULL)
    {
		...
		$contentDetails['content_title'] = $contentTitle; //_CHANGES
		...
		if(isset($userContent))
		{
			...
			if((isset($userContent->is_removed) && $userContent->is_removed == 0) || $isRemoved == 0)
	        {
				...
				//PANKAJ--Update Related data
        	    $userContentOtherInfo = $modelObj->where('shared_content_id', '=', $id)->first();
        		if(isset($userContentOtherInfo))
                {
                	$contentTitleUpdate = array();
           			$contentTitleUpdate['content_title'] = $contentTitle;
                	$userContentOtherInfo->update($contentTitleUpdate);
        		}
			}
		}
		else
    ...
    public function createSentGroupContent($groupId, $memberId, $mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentTitle, $contAttachments, $sharedByUserName, $sharedByUserEmail)
    {
		$contentN = strip_tags($contentText);
        $contentN = preg_replace("/&#?[a-z0-9]+;/i"," ",$contentN);

		$contentTitle = $contentTitle === "" ? substr($contentN, 0, 100): $contentTitle;
    	...
    	if(isset($mainContent))
    	{
			...
			$response = $this->addEditGroupContent(0, $appendedContentText, $contentTitle,  $contentTypeId, $groupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $contentIsLocked, $contentIsShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByUserEmail);
    ...
    public function createSentFolderContent($mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentTitle, $contAttachments, $sharedByUserEmail, $sharedByUserOrEmpId = NULL)
    {
		$contentTitle = $contentTitle === "" ? substr($contentText, 0, 100): $contentTitle;
    	...
    	if(isset($mainContent))
    	{
			...
			// if($contentIsLocked == 0)
			{
				if(isset($senderDepMgmtObj))
				{
					...
					$sentResponse = $senderDepMgmtObj->addEditContent(0, $senderContentText, $contentTitle, $contentType, $sentFolderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $sentFolderContentIsLocked, $sentFolderContentIsShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, ""); // sentToEmail
					
					$sharedContentId = $sentResponse['syncId'];
		            if(is_array($contAttachments) && count($contAttachments) > 0){
						foreach($contAttachments as $contAttachment)
						{
							$serverFileSize = $contAttachment->filesize;
							$cloudStorageTypeId = $contAttachment->att_cloud_storage_type_id;
			...
			if($defFolderId > 0)
			{
	        	...
	        	$response = $this->addEditContent(0, $receiverContentText, $contentTitle, $contentType, $defFolderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $contentIsLocked, $contentIsShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $contentSharedByUserEmail);
	...
	public function sendFolderContentAsReply($contentId, $replaceContentText = NULL, $sendRespectivePush = TRUE, $sendRespectivePushSilently = FALSE)
	{
		...
		if(isset($contentId) && $contentId > 0)
		{
			...
			if(isset($contentObj))
			{
				...
				$contentTitle = $contentObj->content_title;
				...
				if($sharedContentId > 0)// && $contentIsLocked == 0)// && ($contentType == Config::get('app_config.content_type_a')))
				{
					...
					$this->setContentText($contentId, $isFolder, $contentText, $contentTitle);
					...
					if(!isset($sharedContentObj))
					{
						...
						$contentResponse = $senderDepMgmtObj->addEditContent(0, $contentText, $contentTitle, $contentType, $defFolderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $replyContentIsLocked, $replyContentIsShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByUserEmail);
    ...
    public function setContentText($id, $isFolder, $contentText, $contentTitle = "No Title")
    {
    	$contentDetails = array();
		$contentDetails['content_title'] = $contentTitle ;
		$contentDetails['content'] = Crypt::encrypt($contentText);
		$contentDetails['updated_at'] = CommonFunctionClass::getCurrentTimestamp();
		$this->setPartialContentDetails($id, $isFolder, $contentDetails);        	
	}
    ...
    {		
		...
		// PANKAJ -- Remove reciever tag
        $modelContactObj = New AppuserContent;
        $userContentOtherInfo = $modelContactObj->where('shared_content_id', '=', $id)->first();
		if(isset($userContentOtherInfo))
        {
            $contentRecTags = $tagModelObj->ofUserContent($userContentOtherInfo->appuser_content_id)->get();
            foreach ($contentRecTags as $contentRecTag) {
                $tagId = $contentRecTag->tag_id;
                if($tagId > 0 && !in_array($tagId, $tagsArr))
                {
                    $contentRecTag->delete();
                }
            }   
        }
			...
			foreach($tagsArr as $tagId)
            {
                if($tagId > 0 && !in_array($tagId, $existingTags))
                {
            		...
            		if($conName == "")
                	{
                		...
                		if(isset($userContentOtherInfo))
                        {
                            $tagRecieverDetails = $tagDetails;
                        	$tagRecieverDetails['appuser_content_id'] = $userContentOtherInfo->appuser_content_id;
                        	DB::connection($conName)->table($contentTagTableName)->insert($tagRecieverDetails);
                		}
                	}
                }                             
            }
    ...
    public function addEditGroupContent($id, $content, $contentTitle='No Title', $contentTypeId, $groupId, $tagsArr, $isMarked, $createTs, $updateTs, $fromTs, $toTs, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmail = "")
    {
		$response = array();
		$hasConflict = 0;
		$syncId = 0;
			...
			if(isset($participantIdArr) && count($participantIdArr) > 0)
	        {
				...
				foreach($participantIdArr as $participantIndex => $participantId)
	            {
	                if($participantId > 0)
	                {
	                	...
	                	$prtContentResponse = $prtDepMgmtObj->addEditContent($existingParticipantContentId, $appendedContentText, $meetingTitle, $contentTypeId, $usrDefFolderId, $sourceId, $tagsArr, $isMarked, $createTs, $updateTs, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmail);
    	...
    	if(isset($id) && $id > 0)
    	{
	        ...
	        if(isset($videoConferenceResponse))
	        {
                ...
                if($informationStatus > 0)
                {
                    ...
                    $contentAddResponse = $this->addEditContent(0, $contentStr, $conferenceSubject, $contentType, $defFolderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, NULL);
	...
	public function quickCreateAppuserArchiveContent($isFolder = TRUE, $folderOrGroupId = NULL, $contentText, $sharedByUserOrEmpEmail = NULL, $sharedByUserOrEmpId = NULL, $additionalContentParams = NULL, $continuePerformingPush = TRUE)
	{
		...
		if(isset($consFolderOrGroupId) && $consFolderOrGroupId > 0)
		{
            ...
            $contentTitle = strip_tags($contentText);
            $contentTitle = substr($contentTitle, 0, 100);
            $contentTitle = preg_replace("/&#?[a-z0-9]+;/i"," ",$contentTitle);

// 			$contentTitle = substr($contentText, 0, 100);
	                    
	        if($isFolder)
	        {
	            $response = $this->addEditContent(0, $contentText, $contentTitle, $contentType, $consFolderOrGroupId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByUserOrEmpEmail);
	            $newServerContentId = $response['syncId']; 
	        }
	        else
	        {
	            $response = $this->addEditGroupContent(0, $contentText, $contentTitle, $contentType, $consFolderOrGroupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmail); 
	            $newServerContentId = $response['syncId'];
	        }
...
// 		$contentTitle = substr($contentText, 0, 100);
		...
		if(isset($consFolderOrGroupId) && $consFolderOrGroupId > 0)
		{
	        ...
	        if($isFolder)
	        {
	            $response = $this->addEditContent($consContentId, $contentText, $contentTitle,$contentType, $consFolderOrGroupId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByUserOrEmpEmail, $syncWithCloudCalendarGoogle, $syncWithCloudCalendarOnedrive);
	            $newServerContentId = $response['syncId']; 
	        }
	        else
	        {
	            $response = $this->addEditGroupContent($consContentId, $contentText, $contentTitle, $contentType, $consFolderOrGroupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmail); 
	            $newServerContentId = $response['syncId'];
	        }
