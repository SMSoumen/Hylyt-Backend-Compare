		                ...
		                {            
                            ...
                            if (isset($groupContent) && !empty($groupContent->shared_by_email))
                            {
                                if(!empty($groupContent->content_title) && $groupContent->content_title != $contentTitle){
                            		...
                            		$appendedStr .= '-->'.ucwords($sharedByEmpName).' ('.strtolower($sharedByEmpEmail).')'.' ['.$updateTimeStamp.']';
                            		$appendedStr .= 'Title changed from '.$groupContent->content_title;
                                    $content = preg_replace('/(<\/p>)/i', '</p>'.$appendedStr, $content, 1);
                                }
                            }
							
			                $response = $depMgmtObj->addEditGroupContent($id, $content, $contentTitle, $contentTypeId, $groupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmpEmail, $syncWithCloudCalendarGoogle, $syncWithCloudCalendarOnedrive);
