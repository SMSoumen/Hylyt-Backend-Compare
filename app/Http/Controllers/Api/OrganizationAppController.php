...
class OrganizationAppController extends Controller
{
						...
						for($i=0; $i<count($contentArr); $i++)
		                {
		                    ...
		                    if(isset($userContent))
			                {
			                	...
			                	if($contentText != "")
			                	{
			                		...
			                		{
			        					$addedEmployeeContentResponseArr = $this->addEmployeeContent($orgEmployees, $orgId, $userContent, $contentIsLocked, $inpIsShareEnabled, $contentText, $contentAttachmentsArr, $sharedByEmpName, $sharedByEmpEmail, $orgEmpId, $userSession, $contentTitle);   
									...
									if(count($groups) > 0)
			                		{
				        				$addedGroupContentResponseArr = $this->addGroupContent($orgEmpId, $groups, $memberIdArr, $finalShareGroupIdArr, $orgId, $userContent, $contentIsLocked, $inpIsShareEnabled, $contentText, $contentAttachmentsArr, $sharedByEmpName, $sharedByEmpEmail, $userSession, $contentTitle);
			                		...
			                		if(count($appUsers) > 0)
			                		{
			        					$addedFolderContentResponseArr = $this->addUserContent($appUsers, $userContent, $contentIsLocked, $inpIsShareEnabled, $contentText, $contentTitle, $contentAttachmentsArr, $sharedByUserName, $sharedByUserEmail, $userSession);   
    ...
    public function addEmployeeContent($orgEmployees, $orgId, $mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentAttachmentsArr, $sharedByEmpName, $sharedByUserEmail, $sharedByEmpId, $userSession, $contentTitle = NULL)
    {
		...
		foreach($orgEmployees as $orgEmployee)
		{
            ...
            $depMgmtObj->withOrgIdAndEmpId($orgId, $empId);   
       		$addedFolderContentId = $depMgmtObj->createSentFolderContent($mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentTitle, $contentAttachmentsArr, $sharedByUserEmail, $sharedByEmpId);
    ...
    public function addGroupContent($empId, $groups, $memberIdArr, $memberEmpIdArr, $orgId, $mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentAttachmentsArr, $sharedByEmpName, $sharedByUserEmail, $userSession, $contentTitle = NULL)
    {
		...
		foreach($groups as $i => $group)
		{
            ...
            $addedGroupContentId = $depMgmtObj->createSentGroupContent($groupId, $memberId, $mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentTitle, $contentAttachmentsArr, $sharedByEmpName, $sharedByUserEmail);
    ...
    public function addUserContent($appUsers, $mainContent, $contentIsLocked, $contentIsShareEnabled, $contentText, $contentTitle, $contentAttachmentsArr, $sharedByUserName, $sharedByUserEmail, $userSession)
    {
		...
		foreach($appUsers as $appUser)
		{
			...
			//if($userId == 1 || $userId == 4 || $userId == 13 || $userId == 85)
			{
				...
				if(isset($userConstantDetails))
		    	{
                	...
                	$response = $depMgmtObj->addEditContent(0, $appendedContentText, $contentTitle, $contentType, $folderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $contentIsLocked, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, "");
