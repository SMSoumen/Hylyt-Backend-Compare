...
{   
				...
				//if($message != NULL)
				{
					...
					if(isset($content) && $content != "")
					{
			            			...
			            			if(isset($orgUser->organization) && $orgUser->organization->is_active == 1)
			            			{
    									...
    									if(!in_array($orgId, $alreadyUsedOrgIdArr))
    									{
							                ...
							                if(isset($orgEmployee) && $orgEmployee->is_active == 1)
							                {
			            						...
			            						$this->addContentDetails($consOrgAppuser, $content, $from, $orgCode, $attachments);
			            ...
			            if($addContentToRetailProfile && $hasRetailUser)
			            {
							...
							$this->addContentDetails($user, $content, $from, $consOrgSubject, $attachments);
			            }
					}
    ...
    function addContentDetails($user, $content, $sharedByUserEmail, $subject, $attachments = NULL)
    {	
    	$noOrgAttached = TRUE;            	
    	$organization = NULL;
			...
			if(isset($orgUsers) && count($orgUsers) > 0)
			{
				foreach($orgUsers as $orgUser)
				{
					...
					if(isset($orgEmpConstant) && isset($orgEmployee) && $orgEmployee->is_active == 1)
			    	{
	                	...
	                	$response = $depMgmtObj->addEditContent(0, $content, $contentType, $folderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, NULL);	                	
    ...
    public function sendReminderMail()
    {
		...
		if(OrganizationClass::canSendReminderMail($orgId))
		{
			$allUserContents = AppuserContent::select('appuser_id', 'content_type_id', 'content', 'appuser_content_id', 'from_timestamp', 'to_timestamp')
		...
		foreach($organizations as $organization)
		{
			...
			if(OrganizationClass::canSendReminderMail($orgId))
			{
				...
				$allUserContents = $conModelObj->select(['employee_id', 'content_type_id', 'content', 'employee_content_id', 'from_timestamp', 'to_timestamp'])
									->whereNotNull('from_timestamp')
									->whereNotIn('from_timestamp',array("",0))
									->whereIn('content_type_id', $typeRCFil)
									->having('fts', '>=', $minTs)
    ...
    public function sendReminderWebNotifications()
    {
		...
		// if(OrganizationClass::canSendReminderMail($orgId))
		{
			...
			$allUserContents = AppuserContent::select('appuser_id', 'content_type_id', 'content', 'appuser_content_id', 'from_timestamp', 'to_timestamp', 'repeat_duration')
									->whereNotNull('from_timestamp')
									->whereNotIn('from_timestamp',array("",0))
									->whereIn('content_type_id', $typeRCFil)
									->having('fts', '>=', $minTs)
    ...
    public function sendReminderMailTest()
    {
		...
		if(OrganizationClass::canSendReminderMail($orgId))
		{
			$allUserContents = AppuserContent::select('appuser_id','content','appuser_content_id', 'from_timestamp', 'to_timestamp')
									->whereNotNull('from_timestamp')
									->whereNotIn('from_timestamp',array("",0))
									->having('fts', '>=', $minTs)
									->having('fts', '<', $maxTs)
		...
		foreach($organizations as $organization)
		{
			...
			if(OrganizationClass::canSendReminderMail($orgId))
			{
				...
				$conModelObj = New OrgEmployeeContent;
				$conModelObj->setConnection($orgDbConName);
				$allUserContents = $conModelObj->select(['employee_id', 'content', 'employee_content_id', 'from_timestamp', 'to_timestamp'])
									->whereNotNull('from_timestamp')
									->whereNotIn('from_timestamp',array("",0))
									->having('fts', '>=', $minTs)
									->having('fts', '<', $maxTs)
    ...
    public function checkAndDeleteAppuserRemovedContent()
    {
		...
		$orgId = 0;
		$empId = 0;
		$allUserContentsForDelete = AppuserContent::select('appuser_id', 'content', 'appuser_content_id')
		...
		foreach($organizations as $organization)
		{
			...
			$conModelObj = New OrgEmployeeContent;
			$conModelObj->setConnection($orgDbConName);
			$allEmployeeContentsForDelete = $conModelObj->select(['employee_id', 'content', 'employee_content_id'])
														->whereIn('is_removed', [ 1, 2 ])
			...
			$conModelObj = New OrgEmployeeContent;
			$conModelObj->setConnection($orgDbConName);
			$allEmployeeContentsForSoftDeletePermanently = $conModelObj->select(['employee_id', 'content', 'employee_content_id'])
