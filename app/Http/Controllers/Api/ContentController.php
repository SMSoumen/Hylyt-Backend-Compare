...
class ContentController extends Controller
{
    ...
    public function saveContentDetails()
    {
        ...
        $contentTitle = Input::get('content_title'); //_CHANGES
        
        if (trim($contentTitle) === "") {
            $contentTitle = strip_tags($content);
            $contentTitle = substr($content, 0, 100);
            $contentTitle = preg_replace("/&#?[a-z0-9]+;/i"," ",$contentTitle);
        }else{
            $contentTitle = strip_tags($contentTitle);
            $contentTitle = preg_replace("/&#?[a-z0-9]+;/i"," ",$contentTitle);
        }
        ...
        if ($encUserId != "" && $content != "") {
            ...
            if (isset($user)) {
                ...
                if (isset($userContent) && !empty($userContent->shared_by_email))
                {
                    $content = $depMgmtObj->setContentWithTitleChanged($id, $content, $contentTitle, $sharedByUserName, $sharedByUserEmail, $updateTimeStamp);
                }
                ...
                if (!isset($userContent) || (isset($userContent) && $userContent->is_removed == 0)) // && $userContent->is_locked == 0))
                {
                    ...
                    $response = $depMgmtObj->addEditContent($id, $content, $contentTitle, $contentType, $folderId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, "", $syncWithCloudCalendarGoogle, $syncWithCloudCalendarOnedrive, $isRemoved, $removedAt);
        ...
        if($encUserId != "" && $content != "")
        {
            ...
            if(isset($user))
            {
                ...
                if($defFolderId > 0)
                {
                    ...
                    if($consIsFolder)
                    {
                        $contentAddResponse = $depMgmtObj->addEditContent(0, $content,$contentTitle, $contentType, $consFolderOrGroupId, $sourceId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, NULL);
                    }
                    else
                    {
                        $appendedContentText = CommonFunctionClass::getSharedByAppendedString($content, $updateTimeStamp, $sharedByName, $sharedByEmail);

                        $contentAddResponse = $depMgmtObj->addEditGroupContent(0, $appendedContentText,$contentTitle,  $contentType, $consFolderOrGroupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByEmail);
                    ...
                    if($newServerContentId > 0)
                    {
                        ...
                        // if(!$isWebRequest)
                        {
                            ...
                            // $compContentDetails['content_title'] = $contentTitle;
                            if($contentTitle === '' || $contentTitle ===null){
                                $compContentDetails['content_title'] = 'No Title';
                            }else{
                                $compContentDetails['content_title'] = $contentTitle;
                            }
    ...
    public function compileContentDetailsSubView($encOrgId, $depMgmtObj, $isFolderFlag, $id, $isView, $request)
    {
        ...
        $viewDetails['content'] = $userContent;
        $viewDetails['content_title'] = isset($userContent->content_title) ? $userContent->content_title : "";
        $viewDetails['contentTags'] = $contentTags;
        $viewDetails['contentAttachments'] = $contentAttachments;
            ...
            if (isset($user)) {
                ...
                for ($i = 0; $i < count($idArr); $i++) {
                    ...
                    if (isset($content) && $content->is_removed == 0) {
                        ...
                        $conAddDet = $depMgmtObj->addEditContent(0, $contentText, $contentTitle, $contentTypeId, $folderId, $sourceId, $contTagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, NULL, "");
    ...
    public function mergeContents()
    {
        ...
        if ($encUserId != "" && count($idArr) > 0) {
            ...
            if (isset($user)) {
                ...
                // PANKAJ --- check the marge content
                $validMargeReguest = true;
                if (count($idArr) > 1) {
                    $depMgmtObj = new ContentDependencyManagementClass;
                    $depMgmtObj->withOrgKey($user, $encOrgId);
                     for ($i = 0; $i < count($idArr); $i++) {
                        $id = $idArr[$i];
                        $userContent = $depMgmtObj->getContentObject($id, $isFolder);
                        if (isset($userContent) && (!empty($userContent->shared_by_email) || $userContent->shared_content_id > 0) && $validMargeReguest == true) {
                            $validMargeReguest = false;  
                        }
                     }
                }
                if($validMargeReguest == true){
                ...
                if (count($idArr) > 1) {
                    ...
                    for ($i = 0; $i < count($idArr); $i++) {
                        $status = 1;
                        $msg="Content Merged Successfully";
                        $id = $idArr[$i];
                        $userContent = $depMgmtObj->getContentObject($id, $isFolder); 
                        if (isset($userContent) && $userContent->is_removed == 0) {
                            $decContent = "";
                    ...
                    $createTimeStamp = CommonFunctionClass::getCreateTimestamp();
                    $mergedContent->created_at = $createTimeStamp;
                    $mergedContent->updated_at = $createTimeStamp;
                    
                    $updateTs = CommonFunctionClass::getCreateTimestamp();
                    $mergedContent->create_timestamp = $updateTs;
                    $mergedContent->update_timestamp = $updateTs;
                ...
                $status = -1;
                $msg = Config::get('app_config_notif.err_invalid_marge_data');
            }
            ...
            if (isset($user)) {
                ...
                for ($i = 0; $i < count($idArr); $i++) {
                    ...
                    if (isset($content) && $content->is_removed == 0 && isset($actConstant)) {
                        ...
                        $conAddDet = $actDepObj->addEditContent(0, $contentText,$contentTitle, $contentTypeId, $defFolderId, $emailSourceId, NULL, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, NULL, "");
        ...
        if ($encUserId != "" && $contentId != "" && $groupId != "" && $copyToFolderId != "") {
            ...
            if (isset($user)) {
                ...
                if (isset($content) && $content->is_removed == 0) {
                    ...
                    $conAddDet = $depMgmtObj->addEditContent(0, $contentText, $contentTitle, $contentTypeId, $copyToFolderId, $sourceId, $contTagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, NULL, "");
