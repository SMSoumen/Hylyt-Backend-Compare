            ...
            {  
            	...
            	if(isset($groupMember))
            	{
	                ...
	                {	                
		                ...
		                $response = $depMgmtObj->addEditGroupContent($id, $content, $contentType, $groupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $isLocked, $isShareEnabled, $remindBeforeMillis, $repeatDuration, $isCompleted, $isSnoozed, $reminderTimestamp, $removeAttachmentIdArr, $sharedByUserEmail, $syncWithCloudCalendarGoogle, $syncWithCloudCalendarOnedrive);
                	...
                	if($isUserGroupAdmin)
                	{
		                ...
		                if(isset($contentArr) && count($contentArr)>0)
		                {
		                    for($i=0; $i<count($contentArr); $i++)
		                    {
				                ...
				                $id = 0;
				            	$content = urldecode($content);
				            	$contentResponse = $depMgmtObj->addEditGroupContent($serId, $content, $contentType, $groupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $contentIsLocked, $contentIsShareEnabled, $remindBeforeMillis, $repeatDuration, $contentIsCompleted, $contentIsSnoozed, $contentReminderTimestamp, array(), $grpContentSharedByEmail);
					                ...
					                if(isset($contentArr) && count($contentArr)>0)
					                {
					                    for($i=0; $i<count($contentArr); $i++)
					                    {
					                        ...
					                        if(isset($entryObj) && isset($entryObj->content))
					                        {
								            	...
								            	$content = urldecode($content);
								            	$contentResponse = $depMgmtObj->addEditGroupContent($serId, $content, $contentType, $restoredGroupId, $tagsArr, $isMarked, $createTimeStamp, $updateTimeStamp, $fromTimeStamp, $toTimeStamp, $colorCode, $contentIsLocked, $contentIsShareEnabled, $remindBeforeMillis, $repeatDuration, $contentIsCompleted, $contentIsSnoozed, $contentReminderTimestamp, array(), $grpContentSharedByEmail);
						                        $id = $contentResponse["syncId"];
