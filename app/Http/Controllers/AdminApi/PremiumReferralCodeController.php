...
class PremiumReferralCodeController extends Controller
{    
    public $page_icon = "";
	public $page_title = "";
	...
	public function __construct()
    {
        ...
        if(isset($encUserToken) && $encUserToken != "")
        {
            ...
            if(isset($user))
            {
                ...
                $this->module = Config::get('app_config_module.mod_premium_referral_code');
    ...
    {  
        ...
        else
        {
            ...
            $_viewToRender = View::make('premiumreferralcode.index', $data);
     ...
     * @return void
     */
    public function premiumReferralCodeDatatable()
    {
        if(!isset($this->userId) || $this->userId <= 0)
        {
            $msg = Config::get("app_config_notif.err_invalid_user");
        ...
        {
            $templates = PremiumReferralCode::select([ 'referral_code_id', 'referral_code', 'expiration_date', 'allotted_days', 'is_active' ]);
    ...
    public function loadAddEditModal()
    {
        ...
        else
        {
            ...
            if($id > 0)
            {
                $referralCode = PremiumReferralCode::byId($id)->first();
            ...
            $data['page_description'] = $pageName.' '.'Premium Referral Code';
            $data['intJs'] = $intJs;
            $data['usrtoken'] = $this->userToken;
           
            $_viewToRender = View::make('premiumreferralcode.partialview._addEditModal', $data);
            $_viewToRender = $_viewToRender->render();
    ...
    {        
        ...
        else
        {
            ...
            else
            {
                ...
                $referralCode = PremiumReferralCode::create($request->all());
    ...
    public function validatePremiumReferralCode()
    {        
        $id = Input::get('codeId');
        ...
        if($id > 0)
        {
            $refData = PremiumReferralCode::where('referral_code','=',$referralCodeText)
                                        ->where('referral_code_id','!=',$id)
                                        ->get();    
        }
        else
        {
            $refData = PremiumReferralCode::where('referral_code','=',$referralCodeText)->get();    
        }              
