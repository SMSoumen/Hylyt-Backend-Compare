...
class PremiumCouponController extends Controller
{    
    public $page_icon = "";
    public $page_title = "";
    ...
    public function __construct()
    {
        ...
        if(isset($encUserToken) && $encUserToken != "")
        {
            ...
            if(isset($user))
            {
                ...
                $this->module = Config::get('app_config_module.mod_premium_coupon');
    ...
    {  
        ...
        else
        {
            ...
            $_viewToRender = View::make('premiumCoupon.index', $data);
     ...
     * @return void
     */
    public function premiumCouponDatatable()
    {
        if(!isset($this->userId) || $this->userId <= 0)
        {
            $msg = Config::get("app_config_notif.err_invalid_user");
        ...
        {
            $coupons = PremiumCoupon::select([ 'premium_coupon_id', 'coupon_name', 'coupon_count', 'coupon_validity_start_date', 'coupon_validity_end_date', 'subscription_validity_days', 'allotted_space_in_gb', 'coupon_multi_usage_count', 'is_active', 'is_generated' ]);

            return Datatables::of($coupons)
                    ->remove_column('premium_coupon_id')
                    ->remove_column('is_active')
                    ->remove_column('is_generated')
                    ->remove_column('is_active')
                    ->add_column('status', function($coupon) {
                        return sracEncryptNumberData($coupon->premium_coupon_id)."_".$coupon->is_active;
                    })
                    ->add_column('generation', function($coupon) {
                        return $this->getCouponGenerationHtml($coupon->premium_coupon_id, $coupon);
                    })
                    ->add_column('action', function($coupon) {
                        return $this->getCouponDatatableButton($coupon->premium_coupon_id, $coupon);
                    })
                    ->make();
        }
    }
    ...
    {  
        ...
        else
        {
            ...
            if($id > 0)
            {
                $coupon = PremiumCoupon::byId($id)->first();
                if(isset($coupon))
                {
                    $status = 1;
                    ...
                    $_viewToRender = View::make('premiumCoupon.details', $data);
                    $_viewToRender = $_viewToRender->render();
     ...
     * @return void
     */
    public function premiumCouponCodeDatatable()
    {
        if(!isset($this->userId) || $this->userId <= 0)
        {
            $msg = Config::get("app_config_notif.err_invalid_user");
        ...
        else
        {
        	...
        	$baseTableName = 'premium_coupon_codes';

            $couponCodes = PremiumCouponCode::ofCoupon($id)->byUtilizationStatus($utilizationStatus)
        								->joinUtilizedByAppuserTable()
            							->select([ $baseTableName.'.premium_coupon_code_id', 'coupon_code', 'coupon_srno', \DB::raw("IF(is_utilized = 1, 'Utilized', 'Pending') as utilization"), \DB::raw("utilized_at"), \DB::raw("CONCAT(fullname, ' (', email, ') <br/>', country, ' [', user_ip_address, ']') as utilized_by_name") ]);

            return Datatables::of($couponCodes)
                    ->remove_column('premium_coupon_code_id')
                    ...
                    //     return $this->getCouponCodeDatatableButton($couponCode->premium_coupon_code_id);
                    // })
                    ->make();
        }
    }
    ...
    public function loadAddEditModal()
    {
        ...
        else
        {
            ...
            $data['id'] = sracEncryptNumberData($id);
            $data['coupon'] = $coupon;
            $data['page_description'] = $pageName.' '.'Premium Coupon';
            $data['intJs'] = $intJs;
            $data['usrtoken'] = $this->userToken;
           
            $_viewToRender = View::make('premiumCoupon.partialview._addEditModal', $data);
            $_viewToRender = $_viewToRender->render();
    ...
    {        
        ...
        else
        {
            ...
            else
            {
                ...
                $coupon = PremiumCoupon::create($request->all());
    ...
    {        
        ...
        if($id > 0)
        {
            $couponData = PremiumCoupon::where('coupon_code_prefix','=',$couponCodeText)
                                        ->where('premium_coupon_id','!=',$id)
                                        ->get();    
        }
        else
        {
            $couponData = PremiumCoupon::where('coupon_code_prefix','=',$couponCodeText)->get();    
        }              
        ...
        if($id > 0)
        {
            ...
            else
            {
                ...
                if(isset($coupon) && $coupon->is_active == 1 && $coupon->is_generated == 0)
                {
                    ...
                    do
                    {
                        $genCouponCode = CommonFunctionClass::generatePremiumCouponCodeString($couponCodePrefix);
                    ...
                    {
                        for($couponSrNo = 1; $couponSrNo <= $couponMultiUsageCount; $couponSrNo++)
                        {
                            $couponCodeIns = array();
                            $couponCodeIns['premium_coupon_id'] = $id;
                            $couponCodeIns['coupon_code'] = $couponCode;
                            $couponCodeIns['coupon_srno'] = $couponSrNo;

                            $savedCouponCode = PremiumCouponCode::create($couponCodeIns);
                        }
                    }
