...
class OrgContentAdditionController extends Controller
{
    ...
    {                
        ...
        else
        {
            ...
            $contAdditions = $modelObj->select(['mlm_content_addition_id', 'content_title','content_text', \DB::raw("DATE_FORMAT(sent_at, '$dateTimeFormat') as sent_on"), "sent_by"]);//->joinEmployeeTable();
    ...
    {    	
        ...
        if($addContentId != "" && $addContentId > 0 && isset($empIdArr) && count($empIdArr) > 0)
        {
        	...
        	if(isset($addContDetails))
        	{
				...
				$isTest = 0;
				$this->addAndSendContent($employees, $isTest, $contentText, $contentTitle, $contentFileName, $contentFileUrl, $addContentId);	
        	}
        }
    ...
    function addAndSendContent($employees, $isTest, $contentText, $contentTitle, $contentFileName, $contentFileUrl, $addContentId)
    {        
