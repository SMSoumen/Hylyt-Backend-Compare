...
$stackableOptionArr = array();
$stackableOptionArr[1] = 'Yes';
$stackableOptionArr[0] = 'No';
...
<script>
			...
			fields: 
			{
				...
				coupon_code_prefix:
				{
					validators:
					{
						...
						remote:
						{
							...
							url: "{{ route('enterpriseCoupon.validateCouponCodePrefix') }}",
							type: 'POST',
							crossDomain: true,
				...
				allotted_space_in_gb:
				{
					validators:
					{
                        ...
                        integer: {
                            message: 'Allotted Space is not an integer',
                            // The default separators
                            thousandsSeparator: '',
                            decimalSeparator: '.'
                        },
                        between: {
                            min: 1,
                            max: 500,
                            message: 'Allotted Space must be between 1 and 500'
                        }
	             	}
				},
				allotted_user_count:
				{
					validators:
					{
                        ...
                        integer: {
                            message: 'User Count is not an integer',
                            // The default separators
                            thousandsSeparator: '',
                            decimalSeparator: '.'
                        },
                        between: {
                            min: 1,
                            max: 1000,
                            message: 'User Count must be between 1 and 1000'
                        }
	             	}
				},
				coupon_count:
	...
	function saveCouponDetails(frmObj)
	{
		...
		$.ajax({
			type: 'POST',
			crossDomain: true,
			url: siteurl+'/saveEnterpriseCouponDetails',
			dataType: "json",
			data: dataToBeSent,
		})
		.done(function(data){
		...
		<div class="modal-content">
			{!! Form::open(['url' => '', 'class' => 'form-horizontal', 'id' => 'frmSaveCoupon']) !!}
				...
				<div class="modal-body">
					...
					<div class="form-group">
						{!! Form::label('allotted_user_count', 'Allotted User Count*', ['class' => 'col-sm-3 control-label']) !!}
						<div class="col-sm-6">
							{!! Form::text('allotted_user_count', $allottedUserCount, ['class' => 'form-control', 'autocomplete' => 'off']) !!}
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						{!! Form::label('is_stackable', 'Stackable', ['class' => 'col-sm-3 control-label']) !!}
						<div class="col-sm-6">
							{!! Form::select('is_stackable', $stackableOptionArr, $isStackable, ['class' => 'form-control', 'autocomplete' => 'off']) !!}
						</div>
					</div>
					<div class="form-group">
