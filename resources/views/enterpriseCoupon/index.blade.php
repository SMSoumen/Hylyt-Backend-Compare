...
<script>
	...
	$(document).ready(function(){
	    couponTableObj = $('#coupon-table').DataTable({
	        ...
	        serverSide: true,
	         ajax: {
	            url: "{!!  route('enterpriseCouponDatatable') !!}",
	            method: 'POST',
	            data: function ( d ) {
	                d.usrToken = "{{ $usrtoken }}"
            	}
	        ...
	        columns: [
	            ...
	            { data: '6', name: 'status', sortable: false, searchable: false },
	            { data: '7', name: 'generation', sortable: false, searchable: false },
			...
			"columnDefs": [
	        	{
	                ...
	                "render": function ( data, type, row )
	                {
                		...
                		var fnName = "changeEnterpriseCouponStatus";
	...
	function deleteCoupon(id)
	{
		$.ajax({
			type: 'POST',
			url: "{!!  route('enterpriseCoupon.checkAvailForDelete') !!}",
			dataType: "json",
			data:"couponId="+id+"&usrToken="+"{{ $usrtoken }}",
		})
		.done(function(data){
						...
						buttons: {
							...
							callback: function() {
								...
								$.ajax({
									type: 'POST',
									crossDomain: true,
									url: "{!!  route('enterpriseCoupon.delete') !!}",
									dataType: "json",
									data:"couponId="+id+"&usrToken="+"{{ $usrtoken }}",
								})
								.done(function(data){
	...
	function changeEnterpriseCouponStatus(id, statusActive)
	{
		var dataToBeSent = "couponId="+id+"&statusActive="+statusActive+"&usrToken="+"{{ $usrtoken }}";	
		$.ajax({
			type: 'POST',
			crossDomain: true,
			url: "{!!  route('enterpriseCoupon.changeStatus') !!}",
			dataType: "json",
			data: dataToBeSent,
		})
		.done(function(data){
	...
	function generateCouponCodes(id)
	{
		$.ajax({
			type: 'POST',
			url: "{!!  route('enterpriseCoupon.checkAvailForGenerate') !!}",
			dataType: "json",
			data:"couponId="+id+"&usrToken="+"{{ $usrtoken }}",
		})
		.done(function(data){
						...
						buttons: {
							...
							callback: function() {
								...
								$.ajax({
									type: 'POST',
									crossDomain: true,
									url: "{!!  route('enterpriseCoupon.generate') !!}",
									dataType: "json",
									data:"couponId="+id+"&usrToken="+"{{ $usrtoken }}",
								})
								.done(function(data){
	...
	{
		$("#viewId").val(id);
		$("#frmViewEnterpriseCoupon").submit();
	}
