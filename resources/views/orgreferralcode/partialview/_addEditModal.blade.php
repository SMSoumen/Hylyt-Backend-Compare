...
if(isset($referralCode))
{
	...
	$userCount = $referralCode->user_count;
	$allottedDays = $referralCode->allotted_days;
	$quotaAllotted = $referralCode->allotted_quota_in_gb;
...
<script>
				...
				referral_code:
				{
					validators:
					{
						...
						remote:
						{
							...
							url: "{{ route('orgReferralCode.validateReferralCode') }}",
							type: 'POST',
							crossDomain: true,
	...
	function saveReferralCodeDetails(frmObj)
	{
		...
		$.ajax({
			type: 'POST',
			crossDomain: true,
			url: siteurl+'/saveOrgReferralCodeDetails',
			dataType: "json",
			data: dataToBeSent,
		})
		.done(function(data){
		...
		<div class="modal-content">
			{!! Form::open(['url' => '', 'class' => 'form-horizontal', 'id' => 'frmSaveReferralCode']) !!}
				...
				<div class="modal-body">
					...
					<div class="form-group">
           				{!! Form::label('user_count', 'User Count *', ['class' => 'col-sm-3 control-label']) !!}
						<div class="col-sm-6">
							{!! Form::text('user_count', $userCount, ['class' => 'form-control', 'autocomplete' => 'off']) !!}
						</div>
					</div>
					<div class="form-group">
                		{!! Form::label('allotted_quota_in_gb', 'Allotted Quota (In GB) *', ['class' => 'col-sm-3 control-label']) !!}
						<div class="col-sm-6">
