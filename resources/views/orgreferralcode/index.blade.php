...
@section('int_scripts')
<script>
	var moduleName = 'Organization Referral Code';
	var codeTableObj;
	$(document).ready(function(){
	    codeTableObj = $('#referral-code-table').DataTable({
	        ...
	        serverSide: true,
	         ajax: {
	            url: "{!!  route('orgReferralCodeDatatable') !!}",
	            method: 'POST',
	            data: function ( d ) {
	                d.usrToken = "{{ $usrtoken }}"
            	}
	        ...
	        columns: [
	            ...
	            { data: '3', name: 'user_count' },
	            { data: '4', name: 'allotted_quota_in_gb' },
	            { data: '5', name: 'status', sortable: false, searchable: false },
			...
			"columnDefs": [
	        	{
	                ...
	                "render": function ( data, type, row )
	                {
                		...
                		var fnName = "changeOrgReferralCodeStatus";
	...
	function loadQuickAddEditReferralCodeModal(id)
	{
		$.ajax({
			type: 'POST',
			url: "{!!  route('orgReferralCode.loadAddEditModal') !!}",
			dataType: "json",
	...
	function deleteReferralCode(id)
	{
		$.ajax({
			type: 'POST',
			url: "{!!  route('orgReferralCode.checkAvailForDelete') !!}",
			dataType: "json",
			data:"codeId="+id+"&usrToken="+"{{ $usrtoken }}",
		})
		.done(function(data){
						...
						buttons: {
							...
							callback: function() {
								...
								$.ajax({
									type: 'POST',
									crossDomain: true,
									url: "{!!  route('orgReferralCode.delete') !!}",
									dataType: "json",
									data:"codeId="+id+"&usrToken="+"{{ $usrtoken }}",
								})
								.done(function(data){
	...
	function changeOrgReferralCodeStatus(id, statusActive)
	{
		var dataToBeSent = "codeId="+id+"&statusActive="+statusActive+"&usrToken="+"{{ $usrtoken }}";	
		$.ajax({
			type: 'POST',
			crossDomain: true,
			url: "{!!  route('orgReferralCode.changeStatus') !!}",
			dataType: "json",
			data: dataToBeSent,
		})
		.done(function(data){
	...
	<div class="col-md-12">
		...
		<div class="box box-primary">
		    <div class="box-header with-border">
		        <h3 class="box-title">
		        	Organization Referral Code List
