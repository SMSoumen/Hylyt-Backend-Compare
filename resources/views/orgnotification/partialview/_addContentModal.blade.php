...
@php
	$notifId = 0;
   	...
   	{
		$notifId = $id;//$notificationDetails->mlm_notification_id;
		$notifText = $notificationDetails->notification_text;
		$isDraft = $notificationDetails->is_draft;
	}
    ...
    $notifImageUrl = "";
    if(isset($notificationDetails->url))
    	$notifImageUrl = $notificationDetails->url;
@endphp
<div class="modal fade noprint" id="divAddNotificationModal" tabindex="-1" role="dialog" aria-labelledby="divAddNotificationModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="addNotificationTitle">
					Appuser Notification
				</h4>
			</div>
			{!! Form::open(['url' => '', 'class' => 'form-vertical', 'id' => 'frmAddNotification', 'enctype' => "multipart/form-data"]) !!}
			{{ Form::hidden('notif_id', $notifId) }}
			<div class="modal-body" id="divAddNotification">
				@if($isDraft == 1)
					<div >
						<span style="color: red;font-weight: bold;">
							NOTE:
						</span> This Notification would be added for all the verified users.
					</div>
					<br/>
				@endif
				<div class="row">
					<div class="col-md-12 form-group {{ $errors->has('notif_text') ? 'has-error' : ''}}">
						{!! Form::label('notif_text', 'Notification Text *', ['class' => 'control-label']) !!}
						<div class="input-group">
							<span class="input-group-addon">
								<i class="fa fa-bullhorn">
								</i>
							</span>
							{{ Form::textArea('notif_text', $notifText, ['class' => 'form-control', 'id' => 'notif_text', 'autocomplete' => 'off', 'rows'=>'2', $disabledInput]) }}
						</div>
						{!! $errors->first('notif_text', '
						<p class="help-block">
							:message
						</p>') !!}
					</div>
...
<script>
	$(document).ready(function()
	{
		...
		$(".removeImage").click(function() {
			...
			$('#frmAddNotification').formValidation(
				{
					framework: 'bootstrap',
					icon:
					{
						...
						notification_img:
						{
							validators:
							{
								file:
								{
									...
									maxSize: "<?php echo Config::get('app_config.org_mlm_notif_image_filesize_limit'); ?>",
		...
		function registerAttachmentForValidation()
		{
			...
			{
				$('#frmAddNotification').formValidation('revalidateField', 'notification_img');
			});
		}
</script>