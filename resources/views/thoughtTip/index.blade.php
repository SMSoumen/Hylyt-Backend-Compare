...
<script>
	$(document).ready(function(){
	    ...
	    $('#thoughtTips-table').DataTable({
	        processing: true,
	        serverSide: true,
	         ajax: {
	            url: "{!!  route('thoughtTipDatatable') !!}",
	            method: 'GET'
	        },
	        columns: [
	            { data: '0', name: 'for_date' },
	            { data: '1', name: 'thought_tip_text' },
	            { data: '2', name: 'status', sortable: false, searchable: false },
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="box box-primary">
		    <div class="box-header with-border">
		        <h3 class="box-title">
		        	...
		        	@if($modulePermissions->module_add == 1)
			        	&nbsp;&nbsp;
			        	<a href="{{ route('thoughtTip.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="box-body">
			    <div class="table">
			        <table id="thoughtTips-table" class="table table-bordered">
			            <thead>
			                <tr>
			                    <th>For Date</th>
			                    <th>Thought/Tip Text</th>
			                    <th>Status</th>
			                    <th>Action</th>
			                </tr>
			            </thead>
				...
				{{ Form::open(array('url' => route('thoughtTip.show'), 'id' => 'frmViewThoughtTip')) }}
					{!! Form::hidden('thoughtTipId', 0, ['id' => 'viewId']) !!}
				{{ Form::close() }}
				...
				{{ Form::open(array('url' => route('thoughtTip.delete'), 'id' => 'frmDeleteThoughtTip')) }}
					{!! Form::hidden('thoughtTipId', 0, ['id' => 'deleteId']) !!}
				{{ Form::close() }}
			</div>
		</div>
	</div>
